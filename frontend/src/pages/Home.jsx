...
function Home() {
  ...
  }, [search, location, fees, course]);

 const handleCompare = (college) => {
  const user = JSON.parse(localStorage.getItem("user"));
  if (!user) {
    alert("Please login to compare colleges!");
    navigate("/login");
    return;
  }
  ...
  return (
    <div className="min-h-screen" style={{ backgroundColor: "#e8edf2" }}>
      ...
      <div style={{ backgroundColor: "#2c3947" }} className="py-14 px-8 text-center">
        ...
        <p className="text-gray-400 mb-8 text-lg">Search from top colleges across India</p>
      ...
      <div className="px-8 pb-16 max-w-7xl mx-auto">
        {loading ? (
          <div className="text-center mt-20">
            <p className="text-gray-500 text-xl">Searching colleges...</p>
          </div>
        ) : (
          <>
